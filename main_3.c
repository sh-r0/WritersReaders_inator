...
void* writerFunc(void* _args) {
    ...
    while(true) {
        pthread_mutex_lock(&mutex_g);
        while (readersInLib_g != 0 || writersInLib_g != 0) { // while instead of if cause of possibility of 
            writersWaiting_g++;                              // writer being not in queue when first writer is signaled
